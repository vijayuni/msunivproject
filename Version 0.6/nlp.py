...
# download newsfeed from url link and parse xml into json/ dict
def get_news():
    news = []
    for url in urls:
        try:
...
# sentence summarising and paraphrasing transformer function
...
# function to filter similar messages 
...
# recursive function to run filter until no text is left
...
def main():
    ...
    for newsfeed in new_news_bank :
        ...
        i += 1
        #print('get the summary of the text')
        word_frequencies = {}
        ...
        #print('maximum frequancy defined')
                ...
                if word in word_frequencies.keys():
                    if len(sent.split(" ")) < 30:
                        if sent not in sentence_scores.keys():
                            ...
                            sentence_scores[sent] += word_frequencies[word]
        #print('Sentence score defined')
        summary_sentences = heapq.nlargest(2, sentence_scores, key=sentence_scores.get)
        title = summary_sentences
        title_sentences = summary_sentences
        #print('title sentences definde')
      
        # comparing to avoid repetition due to text similarities
